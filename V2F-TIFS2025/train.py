...
def load(feature, D, G):
    states = torch.load('V5F48_0.882.pkl')
...
def train():
    ...
    Rank_loss = lift_4struct(1.2,2,0.0)
    Loss_Fc = nn.CrossEntropyLoss()  # 
    cuda = True if torch.cuda.is_available() else False
    #feature, Discri, generator, Cls = load(feature, Discri, generator, Cls)
    #feature, Discri, generator = load(feature, Discri, generator)
    if cuda:
    ...
    faceroot= './train_V2F_8vox1_label50.txt'#train_face_voice_gallery
    train_dataset=[]
    ...
    for epoch in range(50):
        ...
        for i, data in enumerate(dataloader):
            ...
            #a, f1, f2 = generator(a, f1, f2,'train')
 
            Pv, Pf1, Pf1, a, f1, f2, VF1_Coef, VF2_Coef, Loss_total_Orthogonal = generator(a, f1, f2, a2,'train')
            ...
            predict0, predict = Cls(Pv, Pf1, Pf1, a, f1, f2)

            loss_p = Loss_Fc(predict, label1)


            loss_m = compute_metric_N(label, Rank_loss, a, f1, f2)
            
            loss_total = loss1_g + 2*loss_m + 2*loss_p + 1*Coef_total_loss
