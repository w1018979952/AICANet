...
def compute_metric(labels, func, cAA_an, cAA_ap, f,*a):
    loss = 0
    ...
    loss_Cmodal = func(anchor, positive, cAA_an, cAA_ap, 1-weight_mask, *a)
    loss = loss_Cmodal
    return loss

def compute_metric_N(labels,func, f,*a):
    loss = 0
    device = labels.device
    positive = torch.zeros_like(f).to('cuda')
    for i in range(0,len(a)):
    ...
    loss_Cmodal = func(anchor, positive, 1-weight_mask, *a)
    loss = loss_Cmodal
    return loss
...
def eval(feature_extractor, generator, Cls, Discri, epoch, acc_best):
    ...
    valdataloader = DataLoader(ismydataset, batch_size=50, shuffle=False, num_workers=4)
    ...
    for index, data in enumerate(valdataloader):
        with torch.no_grad():
            ...
            a, f1, f2 = generator.test_forward(a, f1, f2,'test')
